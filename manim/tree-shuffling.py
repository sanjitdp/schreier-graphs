    ...
    def apply_permutation_sequence(self, p_sequence, s):
        ...
        for i in range(len(p_sequence)):
            ...
            if i != len(p_sequence)-1:
                ...
                new_string_eq = MathTex( p_sequence[i+1].symbol, "(", new_string, ")", "=", p_sequence[i+1].apply_to_string(new_string)).to_edge(DOWN)
                self.play(
                    FadeOut(string_eq[:-1]),
...
class CircularTest(Scene):
    def construct(self):
        ...
        self.wait()
        for n in range(1,len(s)+1):
            self.play(C.animate.change_layout(layout={v : C_original[partial_perm(p,v,n)].get_center() for v in C.vertices}), run_time=1)
        self.play(Write(string_eq[4:]))
        self.wait()
